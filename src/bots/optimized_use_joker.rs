...
//     is_melded: bool,
//     meld_cards: Vec<Meld>,
//     rank_cards: Vec<&'a Card>,
//     rank_size: usize,
// }

// impl<'a> UseJokerBot<'a> {
...
//             is_melded: false,
//             meld_cards: Vec::new(),
//             rank_cards: Vec::with_capacity(15),
//             rank_size: 0,
//         }
//     }
...
//     fn get_rank_meld(&self, rank_cards: &[&Card]) -> i32 {
//         let mut melds_val = 0;
//         // Filter jokers and keep references
//         let cards: Vec<&Card> = rank_cards.iter().map(|&c| c).filter(|c| c.rank != Rank::Joker).collect();
...
//     fn get_seq_meld(&self, seq_cards: &[&Card]) -> i32 {
//         let cards: Vec<&Card> = seq_cards.iter().map(|&c| c).filter(|c| c.rank != Rank::Joker).collect();
//         let mut joker_cards: Vec<&Card> = seq_cards.iter().map(|&c| c).filter(|c| c.rank == Rank::Joker).collect();

//         // Local grouping to avoid cloning
//         let mut groups: HashMap<Suit, Vec<&Card>> = HashMap::new();
//         for c in cards {
//             groups.entry(c.suit).or_default().push(c);
//         }

//         let mut all_melds_val = 0; // Just sum value here directly to avoid Meld struct construction logic for now?
//         // Or construct simple Meld structs. But Meld struct requires Card (owned).
//         // For performance, we should avoid creating Meld structs in this check function if possible.
//         // But the logic provided constructs Melds. We will stick to logic but try to minimize cost.
//         // Since we return i32, we don't strictly need Meld objects, but the logic uses `melds_value(&all_melds)`.

//         let mut all_melds: Vec<Meld> = Vec::new(); // Still creating Melds (owned) here for scoring, unavoidable without rewrite.
//         // However, this function is called less often if we prune, or we accept the cost of owned Card creation here only (it's less frequent than the loop itself if we assume valid subsets are rarer).
//         // Wait, this is called inside the loop.
//         // We will proceed with cloning here for now, as the input `seq_cards` is references, saving the pass-in cost.

//         for group in groups.values() {
//                 let mut new_cards: Vec<Card> = group.iter().filter(|c| c.rank != Rank::Ace).map(|&c| c.clone()).collect();
//                 let mut aces_cards: Vec<Card> = group.iter().filter(|c| c.rank == Rank::Ace).map(|&c| c.clone()).collect();
...
//                             let joker_card = joker_cards.pop().unwrap().clone();
//                             delete_invalid_index.push(i);
...
//                     if add_to == -1 { return -1; }
//                     if is_left {
//                         valid_meld_seqs[add_to as usize].insert(0, joker_card.clone());
//                     } else {
//                         valid_meld_seqs[add_to as usize].push(joker_card.clone());
//                     }
//                 }
...
//         if index == hand.len() {
//             if take_rank.count_ones() < 3 || take_seq.count_ones() < 3 { return;}
//             // Using Vec with capacity to avoid re-allocations as much as possible, and using references
//             // to avoid string cloning.
//             // let mut rank_cards2 = Vec::with_capacity(hand.len());
//             // let mut seq_cards: Vec<&Card> = Vec::with_capacity(hand.len());

//             // for i in 0..hand.len() {
//                 // if (take_rank & (1 << i)) != 0 { rank_cards.push(&hand[i]); }
//                 // if (take_seq & (1 << i)) != 0 { seq_cards.push(&hand[i]); }
//             // }
//             return;
...
// impl<'a> BotStrategy for UseJokerBot<'a> {
